...
class VisionEncoderDecoderModel(PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # top_n11=torch.argsort(decoder_outputs.logits.reshape(1,-1),dim=1,descending=True).reshape(-1)[:11].cpu().tolist()
        # soft_max_index.append(top_n11)
        # myGlobal.set_value('soft_max_index',soft_max_index)
        if labels is not None:
            logits = decoder_outputs.logits if return_dict else decoder_outputs[0]
            loss_name=myGlobal.get_value('loss_name')
            b = myGlobal.get_value('b')
            if loss_name=='CE':
                loss_fct = CrossEntropyLoss()
            elif loss_name=='DRSL':
                loss_fct = DRSL(b=b)
            elif loss_name=='DRSL3':
                start=myGlobal.get_value('start')
                end=myGlobal.get_value('end')
                loss_fct = DRSLx(b=b,start=start,end=end)
