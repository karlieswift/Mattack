...
coco_path='/public/home/mswanghao/TorchProject/vitgpt2/train2017'


coco_path='/public/home/mswanghao/image_caption/natural_images1000'
coco_images = random.sample(os.listdir(coco_path), 100)
...
for loss_name in os.listdir(model_path):
    ...
    for path in tqdm(coco_images):
        ...
        index_counts += 1
        image_path = os.path.join(coco_path, path)
        ...
        res_all.append([loss_name, path, adv_res])
pd.DataFrame(res_all, columns=['loss_name', 'pic_name', 'answers']).to_csv(
    './adv_vitgpt2/adv_2_{}.csv'.format(str(time.time())),
    index=False)
