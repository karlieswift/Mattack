...
stop_words = ['plane', 'jetliner', 'flying', 'air', 'sky', 'flying','airplane']#非目标攻击
myGlobal._init()
...
for p1 in os.listdir(pic_path):
    ...
    for epoch in range(1, 50):
        ...
        labels = tokenizer(
            ...
            'this is a jetliner or plane or airplane in the air.',
            return_tensors="pt",
        ).input_ids.to(device)
        print("labels:", labels)
        ...
        if epoch == 1:
            epoch_1 = generated_text
            print("第{}次攻击: 生成内容:{}".format(epoch, generated_text))
        else:
            reference=[word_tokenize(epoch_1)]
...
pd.DataFrame(res_all, columns=['pic_name', 'answer']).to_csv('./adv_no_target_metor_plane_CE.csv', index=False)
